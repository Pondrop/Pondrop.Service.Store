...
public class UpdateStoreAddressCommandHandler : StoreCommandHandler<UpdateStoreAddressCommand, Result<StoreRecord>>
{
    ...
    public UpdateStoreAddressCommandHandler(
        ...
        ILogger<UpdateStoreAddressCommandHandler> logger) : base(storeUpdateConfig, daprService, logger)
    {
    ...
    public override async Task<Result<StoreRecord>> Handle(UpdateStoreAddressCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (storeStream.Version >= 0)
            {
                ...
                {
                    store.Apply(new UpdateStoreAddress(
                        command.Id,
                        command.StoreId,
